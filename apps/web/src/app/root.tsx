...
// Wrapper to ensure React Router context is available
function SafeSessionProvider({ children }: { children: ReactNode }) {
  const [isReady, setIsReady] = useState(false);
  
  useEffect(() => {
    // Delay slightly to ensure React Router context is initialized
...
export const ClientOnly: React.FC<ClientOnlyProps> = ({ loader }) => {
  ...
  if (!isMounted) {
    // Render a placeholder on server and before mount
...
export function useHmrConnection(): boolean {
  // SSR-safe: use useSyncExternalStore pattern
  // During SSR, always return true (no HMR on server)
  if (typeof window === 'undefined') {
    return true;
  }
  
  // Use a simple ref that doesn't trigger useState during hydration
