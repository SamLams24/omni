...
const ROLE_PERMISSIONS: Record<string, readonly string[]> = {
  ...
  SELLER: ["businesses.read", "businesses.create", "businesses.update", "seller.dashboard.access"],
...
async function main() {
  ...
  // --- Roles & permissions -------------------------------------------------
  const permissionsByKey: Map<string, Awaited<ReturnType<typeof prisma.permission.upsert>>> = new Map(
    await Promise.all(
  ...
  const admin = await prisma.user.upsert({
    ...
    update: {},
    create: { name: "OMNI Admin (dev)", email: "admin@omni.dev", passwordHash: devPasswordHash },
  });
  ...
  const vendorUser = await prisma.user.upsert({
    ...
    update: {},
    create: { name: "Vendeur Démo", email: "vendor@omni.dev", passwordHash: devPasswordHash },
  });
  await prisma.userRole.upsert({
  ...
  const buyerUser = await prisma.user.upsert({
    ...
    update: {},
    create: { name: "Acheteur Démo", email: "buyer@omni.dev", passwordHash: devPasswordHash },
  });
  await prisma.userRole.upsert({
  ...
  console.log("Dev logins (development only, see docs/setup/admin-account.md):");
